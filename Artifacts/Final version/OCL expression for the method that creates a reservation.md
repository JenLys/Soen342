...
Context BookingDB::create_reservation(fname: String, lname: String, age: Integer, selected_option: String, user_id: String, current: boolean)

<ins> pre </ins> <br>
--the passed user exists <br>
self.User ---> exists(u:User | u.user_id = user_id) <br>
--the option selected is valid (it exists (existing id passed) and is not void) <br>
self.selected_option <> null <br>
and <br>
...
self.trip ---> size() = self.trip@pre + 1 <br>
